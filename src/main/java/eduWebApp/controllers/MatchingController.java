...
import eduWebApp.domain.MatchingQuestions;

@Controller
@RequestMapping("/english")
public class MatchingController {
	
	private MatchingQuestions matchingQuestions = new MatchingQuestions();
	private MatchingQuestionPair match;
	...
	public String english(Model model, @ModelAttribute("App") String App, RedirectAttributes rattrs) {
		...
		matchingQuestions = new MatchingQuestions();
		...
		model.addAttribute("timerStart", "14");
			...
			match = matchingQuestions.getEasyDifficultyQuestion();
			...
			// allows score to be carried over if the user starts with the math game
			if (EducationalGameApplication.isEnglishStarted() == false) {
				model.addAttribute("score",(EducationalGameApplication.getEnglishScore()+EducationalGameApplication.getMathScore()));
			}
			// shows the user what question they are answering 
		...
		// medium Difficulty  Questions
				...
				match = matchingQuestions.getMediumDifficultyQuestion();
				model.addAttribute("name",match.getName());
				model.addAttribute("ans1",match.get_ans1());
				model.addAttribute("ans2",match.get_ans2());
				model.addAttribute("ans3",match.get_ans3());
		...
		//Hard Question Difficulty 
			...
			match = matchingQuestions.getHardDifficultyQuestion();
			model.addAttribute("name",match.getName());
			model.addAttribute("ans1",match.get_ans1());
			model.addAttribute("ans2",match.get_ans2());
			model.addAttribute("ans3",match.get_ans3());
...
public String opt5(Model model,RedirectAttributes rattrs, @RequestParam String timer,@RequestParam String timerRemaining) {
	EducationalGameApplication.setTimerRemaining(Integer.parseInt(timerRemaining));
			...
			if (match.isCorrect(match.get_ans5())) {
				if (EducationalGameApplication.getCounter3() == 0) {
...
// when the timer goes to zero the user will see he next question
...
// transitioning from mathcing game to numbers game 
